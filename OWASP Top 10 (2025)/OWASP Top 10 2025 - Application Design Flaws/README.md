...
![](2025-12-19_13-45.png)

A: `THM{V3RB0S3_3RR0R_L34K}`
...
By checking out the code, we observe that if we pass `data  = debug` we will get a different response than usual.

![](2025-12-19_14-02.png)

We can do this using `curl`

`curl -X POST http://10.81.131.27:5003/api/process \   -H "Content-Type: application/json" \   -d '{"data":"debug"}'`

A: `THM{SUPPLY_CH41N_VULN3R4B1L1TY}`
...
Check the `/static/js/decrypt.js` file found when viewing the page source.

We have all the details we need here:

![](2025-12-19_14-08.png)

We then go to an online tool and decrypt it.

![](2025-12-19_14-14.png)

A: `THM{CRYPTO_FAILURE_H4RDCOD3D_K3Y}`
...
Find the path to the messages of admin: `10.81.131.27:5005/api/messages/admin`

A: `THM{1NS3CUR3_D35IGN_4SSUMPT10N}`